...
GxEPD2_3C<GxEPD2_420c, /*GxEPD2_420c::HEIGHT*/30> display(GxEPD2_420c(/*CS=D8*/ CS_PIN, /*DC=D3*/ DC_PIN, /*RST=D4*/ RES_PIN, /*BUSY=D2*/ BUSY_PIN));
U8G2_FOR_ADAFRUIT_GFX u8g2Fonts;

#define FONT_TEXT u8g2_font_wqy16_t_gb2312 // 224825bytes，最大字库（天气描述中“霾”，只有此字库中有）
#define FONT_SUB u8g2_font_wqy12_t_gb2312a // 次要字体，u8g2最小字体
...
extern struct tm tmInfo; // 日历显示用的时间
const int _week_1st = 0;
...
void draw_cal_days(bool partial) {
    ...
    for (size_t iDay = 0; iDay < totalDays; iDay++) {
        ...
        if (tmInfo.tm_year + 1900 == holidays.year && tmInfo.tm_mon + 1 == holidays.month) {
            uint8_t holidayIndex = 0;
            for (; holidayIndex < holidays.length; holidayIndex++) {
                if (abs(holidays.holidays[holidayIndex]) == (iDay + 1)) {
                    // 显示公休、调班logo和颜色
                    u8g2Fonts.setFont(u8g2_font_open_iconic_all_1x_t);
                    if (holidays.holidays[holidayIndex] > 0) { // 公休
                        color = GxEPD_RED;
                        u8g2Fonts.setForegroundColor(color);
                        u8g2Fonts.setBackgroundColor(GxEPD_WHITE);
                        u8g2Fonts.drawUTF8(x + 44, y + 11, "\u006c");
                    } else if (holidays.holidays[holidayIndex] < 0) { // 调班
                        color = GxEPD_BLACK;
                        u8g2Fonts.setForegroundColor(color);
                        u8g2Fonts.setBackgroundColor(GxEPD_WHITE);
...
void si_screen() {
    ...
    display.init(0);          // 串口使能 初始化完全刷新使能 复位时间 ret上拉使能
